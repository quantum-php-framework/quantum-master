	...
	function paginate($passed_page, $passed_ipp)
	{
		...
		if($this->num_pages > 10)
		{
			...
			for($i=1;$i<=$this->num_pages;$i++)
			{
				...
				{
					$this->return .= ($i == $this->current_page And $passed_page != 'All') ? "":"";
				}
				if($this->range[$this->mid_range-1] < $this->num_pages-1 And $i == $this->range[$this->mid_range-1]) $this->return .= " ... ";
			}
			$this->return .= (($this->current_page != $this->num_pages And $this->items_total >= 10) And ($passed_page != 'All')) ? "":"";
			$this->return .= ($passed_page == 'All') ? "":"";
		}
		else
		{
			for($i=1;$i<=$this->num_pages;$i++)
			{
				$this->return .= ($i == $this->current_page) ? "":"$i";
			}
